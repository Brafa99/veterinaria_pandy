...
async function ejecutarMigracion() {
    ...
    for (const fila of sqlData) {
        const r = Array.from(fila);
        if (!r[0]) continue;

        const id_h = r[0].toString();
        const descripcion = r[1] || "";
        const fechaStr = r[2] || "";
        const tipo_historial = r[3] || "Consulta Medica";
        const id_c = r[4] ? r[4].toString() : "";
        const precio = parseInt(r[6]) || 0;

        try {
            // 1. Buscar el cliente en Firestore
            let c = {};
            if (id_c) {
                const snap = await db.collection('clientes').where('id_cliente', '==', id_c).limit(1).get();
                if (!snap.empty) {
                    c = snap.docs[0].data();
                }
            }
            ...
            // 2. Procesar fecha
            let fechaFirebase = admin.firestore.Timestamp.now();
            if (fechaStr && fechaStr !== "0000-00-00") {
                const parts = fechaStr.split('-');
                // Formato esperado YYYY-MM-DD
                const dateObj = new Date(parts[0], parts[1] - 1, parts[2]);
                if (!isNaN(dateObj.getTime())) {
                    fechaFirebase = admin.firestore.Timestamp.fromDate(dateObj);
                }
            }

            // 3. Crear objeto de historial
            const docData = {
                ci: c.ci || "",
                color: c.color || "N/A",
                correo: c.correo || "",
            ...
            // 4. Añadir al Batch
            const docRef = db.collection('historial_v2').doc(id_h);
            batch.set(docRef, docData);
            countInBatch++;

            // Si llegamos a 400 (margen de seguridad), enviamos el lote
            if (countInBatch >= 400) {
                await batch.commit();
                batch = db.batch();
                countInBatch = 0;
                console.log(`📦 Lote de 400 registros enviado...`);
            }

            procesados++;

        } catch (e) {
            console.error(`❌ Error en ID ${id_h}:`, e.message);
            errores++;
        }
    }

    // Enviar los registros restantes
    if (countInBatch > 0) {
        await batch.commit();
    }

    console.log(`\n🏁 Resumen: ${procesados} exitosos, ${errores} fallidos.`);
}

function corregirTexto(texto) {
    if (!texto) return "";
    try {
        // Importante: Asegúrate de que 'texto' sea un String antes de Buffer
        return Buffer.from(texto.toString(), 'latin1').toString('utf8');
    } catch (e) { 
        return texto; 
    }
}
